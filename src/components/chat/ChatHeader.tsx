...
import { MoreVertical, Phone, Video, ArrowLeft, Search, Pin, Image, Users } from "lucide-react";
...
export default function ChatHeader({ 
  ...
  const onlineCount = isGroup && onlineUsers 
    ? conversation.participants?.filter((p: any) => onlineUsers.has(p.user_id)).length || 0
    : 0;
  const totalMembers = isGroup ? conversation.participants?.length || 0 : 0;
  ...
  const getSubtitle = () => {
    if (isTyping) {
      if (isGroup && typingUsers?.[0]?.username) {
    ...
    if (isGroup) {
      if (onlineCount > 0) {
        return `${totalMembers} thành viên, ${onlineCount} online`;
      }
      return `${totalMembers} thành viên`;
    }
    
    return isOnline ? "Đang hoạt động" : formatLastSeen(otherUser?.last_seen);
  };
  ...
  return (
    <>
      <div className="h-14 bg-[hsl(var(--wa-header-bg))] px-2 md:px-4 flex items-center gap-2 shadow-sm">
        ...
        <div 
          className="flex items-center gap-3 flex-1 min-w-0 cursor-pointer hover:bg-white/5 rounded-lg px-2 py-1 -ml-2"
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-primary-foreground/70 truncate flex items-center gap-1">
              {isTyping ? (
                <>
                  <span className="text-[hsl(var(--wa-light-green))]">{getSubtitle()}</span>
        ...
        <div className="flex items-center">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48 bg-card">
              ...
              {!isGroup && <DropdownMenuItem>Xem hồ sơ</DropdownMenuItem>}
